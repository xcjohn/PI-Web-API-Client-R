notificationRuleApi <- R6Class("notificationRuleApi",
	private = list(),
	public = list(
		serviceBase = NULL,
		authType = NULL,
		...
		getNotificationRules = function(webId, selectedFields, webIdType) {
			...
			localVarPath <- paste(c(self$serviceBase, '/notificationrules/', webId), collapse = "")
			...
			if (res$status == 200) {
				attr(contentResponse, "className") <- "PINotificationRule"
			}
		...
		getNotificationRuleSubscribers = function(webId, selectedFields, webIdType) {
			...
			localVarPath <- paste(c(self$serviceBase, '/notificationrules/', webId, '/notificationrulesubscribers'), collapse = "")
			if (missing(selectedFields) == FALSE && is.null(selectedFields) == FALSE && selectedFields != "") {
				qs$add('selectedFields', selectedFields, FALSE);
				if (is.character(selectedFields) == FALSE) {
					return (print(paste0("Error: selectedFields must be a string.")))
			...
			contentResponse <- content(res)
			if (res$status == 200) {
				attr(contentResponse, "className") <- "PIItemsNotificationRuleSubscriber"
			}
			return (contentResponse)
		},
		getNotificationRulesQuery = function(databaseWebId, maxCount, query, selectedFields, startIndex, webIdType) {
			qs <- customQueryString$new()
			localVarPath <- paste(c(self$serviceBase, '/notificationrules/search'), collapse = "")
