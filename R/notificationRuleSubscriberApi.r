notificationRuleSubscriberApi <- R6Class("notificationRuleSubscriberApi",
	private = list(),
	public = list(
		serviceBase = NULL,
		authType = NULL,
		...
		getNotificationRuleSubscriberByPath = function(path, selectedFields, webIdType) {
			...
			localVarPath <- paste(c(self$serviceBase, '/notificationrulesubscribers'), collapse = "")
			...
			if (res$status == 200) {
				attr(contentResponse, "className") <- "PINotificationRuleSubscriber"
			}
		...
		getNotificationRuleSubscriber = function(webId, selectedFields, webIdType) {
			...
			localVarPath <- paste(c(self$serviceBase, '/notificationrulesubscribers/', webId), collapse = "")
			if (missing(selectedFields) == FALSE && is.null(selectedFields) == FALSE && selectedFields != "") {
				qs$add('selectedFields', selectedFields, FALSE);
				if (is.character(selectedFields) == FALSE) {
					return (print(paste0("Error: selectedFields must be a string.")))
		...
		getNotificationRuleSubscribers = function(webId, selectedFields, webIdType) {
			...
			localVarPath <- paste(c(self$serviceBase, '/notificationrulesubscribers/', webId, '/notificationrulesubscribers'), collapse = "")
			if (missing(selectedFields) == FALSE && is.null(selectedFields) == FALSE && selectedFields != "") {
				qs$add('selectedFields', selectedFields, FALSE);
				if (is.character(selectedFields) == FALSE) {
					return (print(paste0("Error: selectedFields must be a string.")))
